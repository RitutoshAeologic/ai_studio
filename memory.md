...
35. **Robust Zero-False-Positive Image Validation**:
   - Calibrated `ImageValidator` ([lib/core/utils/image_validator.dart](file:///Users/mac/StudioProjects/ai_studio/lib/core/utils/image_validator.dart)) to ensure 100% of valid photos, light-themed app screenshots (e.g. LingoOffline), vector art (e.g. Panda onboarding illustration), and mobile UI designs pass without false rejections.
   - Enforces essential safety filters: file size bounds (512 B to 20 MB), file format validation (JPEG, PNG, WEBP, HEIC), file decode bitmap verification, and pure single-color solid blank canvas detection (`stdDevLuminance < 0.001`).
